...
const VideosDashboard = ({ onClose }) => {
  ...
  const sendVideoProgress = async (videoId, progress) => {
    const email = localStorage.getItem("userEmail");
    if (!email) return;

    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/user/videos/progressUpdate/victims/?email=${encodeURIComponent(email)}&video=${videoId}&progress=${progress}`,
        { method: "GET" }
      );
  ...
  useEffect(() => {
    ...
    const url = email
      ? `http://127.0.0.1:8000/api/user/videos/progress/victims/?email=${encodeURIComponent(email)}`
      : `http://127.0.0.1:8000/api/videos/theme/victims`;
    ...
    fetch(url)
      ...
      .then(data => {
        console.log("Videos loaded (with or without progress):", data);
        // Normalize shape so the UI always receives `id`, `title`, `description`, `thumbnail`, `video`, `progress`
        const normalized = data.map(item => ({
          id: item.id || item._id || item._id?.toString(),
          title: item.title,
          description: item.description,
          thumbnail: item.thumbnail,
          video: item.video || item.video_url || item.video_url,
          status: item.status || 'PENDING',
          progress: item.progress || 0
        }));

        setVictimStories(normalized);
      })
      .catch(err => console.error("Error fetching videos/progress:", err))
        ...
        <main style={{
          ...
          <div style={{
            ...
            {currentStories.map(story => {
              // Display the greater of backend progress and live progress
              const displayProgress = Math.max(story.progress || 0, videoProgress[story.id] || 0);
      ...
      {showVideoPlayer && selectedVideo && (
        <VideoPlayerModal video={selectedVideo}
          onClose={() => {
            ...
            const progress = videoProgress[selectedVideo.id] || 0;
            sendVideoProgress(selectedVideo.id, progress);
          }}
