...
const VRDashBoard = ({ onClose }) => {
  ...
  const sendVideoProgress = async (videoId, progress) => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/user/videos/progressUpdate/disasters/?email=${encodeURIComponent(email)}&video=${videoId}&progress=${progress}`,
        { method: "GET" }
      );
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/user/videos/progress/disasters/?email=${encodeURIComponent(email)}`)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
        return res.json();
      })
      .then(data => {
        console.log("Videos with User Progress:", data);
        setVictimStories(data); // each item already has progress
      })
