...
import T5Intro from '../../public/assets/Theme5/T5Intro.mp4'
...
const T5Page = () => {
    ...
    // If already in fullscreen, start playback directly without showing the button
    if (document.fullscreenElement) {
      setUserInteracted(true);
      setShowPlayButton(false);
      setIsPlaying(true);
    ...
    return (
        <div className="w-full min-h-screen overflow-hidden bg-black relative">
                ...
                <div className="absolute inset-0 flex items-end justify-center pb-[env(safe-area-inset-bottom)] pb-12 z-40">
                    <div className="text-center px-4 py-10">
                        <p className="text-white text-sm md:text-base lg:text-lg mb-4 opacity-80 leading-snug" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.8)' }}>
                            {document.fullscreenElement ? 'Ready to begin your journey?' : 'Go fullscreen for the best experience'}
                        </p>
                        <button
                        ...
                        >
                            {document.fullscreenElement ? 'Enter Our World' : 'Enter Fullscreen'}
                        </button>
                    </div>
                </div>
            )}
            ...
            {showDashboard && (
                ...
                >
                    <BloosomDashboard onClose={closeDashboard} onPlayVideo={playVideo} />
                </div>
            )}
        </div>
    );
